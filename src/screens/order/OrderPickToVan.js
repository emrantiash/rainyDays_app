...
import { storeOrderData,branchOrderSacBarcodeDetails,storePicktoVanData } from '../../redux/slices/orderSlice';
...
export default function OrderPickToVanScreen({ navigation }) {
    ...
    const dataset = useSelector((state)=>state.orderReducer.branchHavetoScan)
    const [isLoading, setIsLoading] = useState(true)

    // console.log("===data====",dataset,branchHavetoScan)

    const MakeApiCall = () =>{
        dispatch(branchOrderSacBarcodeDetails(token)).then(function(e){
            console.log(e)
    ...
    const renderItem = ({ item }) => (
        ...
        >
            ...
            <WDIcon name="shop" size={_size} color="red" />
    ...
    return (
        ...
        >
           <Text style={styles.subHeader}>Store Pick </Text>
           {
            isLoading &&
            <WDSkeleton />
           }
